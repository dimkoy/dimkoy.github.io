...
import { formatPeriodParts } from "@/lib/format";
import { localePath, type Locale } from "@/lib/i18n/config";
import type { Dictionary } from "@/lib/i18n/dictionaries";

function Period({ start, end, present, locale }: { start: string; end?: string; present: string; locale: Locale }) {
  const p = formatPeriodParts(start, end, present, locale);
  return (
    <div>
      <time dateTime={start}>{p.start}</time>
      {p.end !== undefined && (
        <>
          {" – "}
          {p.open ? <span>{p.end}</span> : <time dateTime={end}>{p.end}</time>}
        </>
      )}
    </div>
  );
...
export function Experience({ items, locale, dict }: { items: Exp[]; locale: Locale; dict: Dictionary }) {
  return (
    <ol className="space-y-8">
      {items.map((e) => (
        <li key={e.company}>
          <article className="grid gap-1 sm:grid-cols-[7.5rem_1fr] sm:gap-6">
            <div className="text-sm text-muted tnum">
              {e.roles.map((r) => <Period key={r.title} start={r.start} end={r.end} present={dict.home.present} locale={locale} />)}
            </div>
